...
6. `Coding-Agent Runner`
   - Creates workspace.
   - Builds prompt from issue + workflow template.
   - Launches the configured coding-agent runtime.
   - Streams agent updates back to the orchestrator.
   - The runtime is selected by `agent.runtime` (see §5.3.5).
...
- `kind` (string)
  ...
  - Supported values:
    - `linear` — Linear issue tracker (see §5.3.1 fields below and §11.2–§11.5). REQUIRED fields: `tracker.api_key`, `tracker.project_slug`.
    - `markdown` — Local Markdown files as issues (see §5.3.8). REQUIRED fields: `tracker.markdown.root`.
    - `openspec` — OpenSpec change-proposal layout (see §5.3.9). REQUIRED fields: `tracker.openspec.root` (readable directory).
    - `jira` — JIRA Cloud REST v3 (see §5.3.10). REQUIRED fields: `tracker.jira.endpoint`, `tracker.jira.email`, `tracker.jira.api_token`, `tracker.jira.project_key`.
    - `memory` — In-memory tracker for testing only; not intended for production use.
- `endpoint` (string)
...
- `between_turns` (multiline shell script string, OPTIONAL)
  - Default: empty (disabled).
  - Runs after each agent turn that completes normally, before the next turn begins.
  - The hook executes with the workspace directory as `cwd` and honours `hooks.timeout_ms`.
  - On non-zero exit (including timeout), the captured stdout is appended to the next turn's
    continuation prompt under a `Validation feedback` heading (see §12.3 for continuation
    semantics). A timeout is reported in that section as `<hook timed out after Nms>`.
  - The hook NEVER aborts the run; turn processing continues regardless of hook exit status.
  - Stdout is truncated to approximately 4 KiB; a truncation marker is appended when truncated.
  - If `hooks.between_turns` is non-empty, `hooks.timeout_ms` MUST be greater than zero.
...
- `runtime` (string)
  - REQUIRED.
  - Selects the coding-agent runtime. Allowed values: `codex` (default), `claude`.
  - Validation: MUST be one of the allowed values; preflight rejects any other value.
...
This section is only consulted when `agent.runtime == "codex"`.
...
This section is only consulted when `agent.runtime == "claude"`.
...
- `command` (string shell command)
  ...
  - The runtime launches this command via `bash -lc` in the workspace directory.
  - The launched process MUST accept a newline-delimited JSON message on stdin and emit
    newline-delimited JSON on stdout per the Claude stream-json protocol (see §10.8).
- `model` (string, OPTIONAL)
  - When present, passed through to the `--model` flag of the `claude` CLI invocation.
- `permission_mode` (string, OPTIONAL)
  - When present, passed through to the `--permission-mode` flag of the `claude` CLI invocation.
- `turn_timeout_ms` (integer)
  - Default: `3600000` (1 hour)
- `read_timeout_ms` (integer)
  - Default: `5000`
...
#### 5.3.8 `tracker.markdown` (object)

This section is only consulted when `tracker.kind == "markdown"`.

Fields:

- `root` (path string)
  - REQUIRED when `tracker.kind == "markdown"`.
  - No default.
  - Path resolution rules are the same as `workspace.root`: `~` is expanded; relative paths are
    resolved relative to the directory containing `WORKFLOW.md`; the effective path is normalized
    to an absolute path before use.

#### 5.3.9 `tracker.openspec` (object)

This section is only consulted when `tracker.kind == "openspec"`.

Fields:

- `root` (path string)
  - Default: `openspec`
  - Resolved relative to the directory containing `WORKFLOW.md` when a relative path is given.
  - The effective path is normalized to an absolute path before use.
  - Preflight rejects this value if the resolved path is not a readable directory.

#### 5.3.10 `tracker.jira` (object)

This section is only consulted when `tracker.kind == "jira"`.

Fields:

- `endpoint` (URL string)
  - REQUIRED when `tracker.kind == "jira"`.
  - Base URL of the JIRA Cloud instance (for example `https://example.atlassian.net`).
- `email` (string)
  - REQUIRED when `tracker.kind == "jira"`.
  - JIRA account email used for Basic authentication.
  - MAY be a literal value or a `$VAR_NAME` reference.
- `api_token` (string)
  - REQUIRED when `tracker.kind == "jira"`.
  - JIRA API token used for Basic authentication.
  - MAY be a `$VAR_NAME` reference.
  - Canonical environment variable: `JIRA_API_TOKEN`.
  - If `$VAR_NAME` resolves to an empty string, treat the token as missing.
- `project_key` (string)
  - REQUIRED when `tracker.kind == "jira"`.
  - JIRA project key (for example `ABC`) used to scope issue queries.
- `jql_override` (string, OPTIONAL)
  - When present, replaces the default JQL query
    `project = <project_key> AND status in (<active_states>)` with the supplied JQL string.
  - Implementations SHOULD document the expected field projection.
...
- `codex.command` is present and non-empty when `agent.runtime == "codex"` (or `agent.runtime`
  is absent/defaulted to `codex`).
- `agent.runtime` MUST be present and MUST be one of `{codex, claude}`; preflight rejects any
  other value.
- Tracker-kind-specific REQUIRED fields MUST be present:
  - `tracker.kind == "markdown"`: `tracker.markdown.root` MUST be present and non-empty.
  - `tracker.kind == "jira"`: `tracker.jira.endpoint`, `tracker.jira.email`,
    `tracker.jira.api_token`, and `tracker.jira.project_key` MUST all be present after `$`
    resolution.
  - `tracker.kind == "openspec"`: the resolved `tracker.openspec.root` directory MUST be
    readable.
- If `hooks.between_turns` is non-empty, `hooks.timeout_ms` MUST be greater than zero.
...
- `tracker.kind`: string, REQUIRED, supported values: `linear`, `markdown`, `openspec`, `jira`, `memory` (test-only)
- `tracker.endpoint`: string, default `https://api.linear.app/graphql` when `tracker.kind=linear`
- `tracker.api_key`: string or `$VAR`, canonical env `LINEAR_API_KEY` when `tracker.kind=linear`
- `tracker.project_slug`: string, REQUIRED when `tracker.kind=linear`
- `tracker.active_states`: list of strings, default `["Todo", "In Progress"]`
- `tracker.terminal_states`: list of strings, default `["Closed", "Cancelled", "Canceled", "Duplicate", "Done"]`
- `tracker.markdown.root`: path resolved to absolute, REQUIRED when `tracker.kind=markdown`, no default
- `tracker.openspec.root`: path resolved to absolute, default `openspec` (resolved relative to WORKFLOW.md dir), REQUIRED readable directory when `tracker.kind=openspec`
- `tracker.jira.endpoint`: URL string, REQUIRED when `tracker.kind=jira`
...
- `tracker.jira.project_key`: string, REQUIRED when `tracker.kind=jira`
- `tracker.jira.jql_override`: string, OPTIONAL — replaces default JQL when `tracker.kind=jira`
...
- `hooks.between_turns`: shell script or null, default empty (disabled)
...
- `codex.approval_policy`: Codex `AskForApproval` value, default implementation-defined (only when `agent.runtime=codex`)
- `codex.thread_sandbox`: Codex `SandboxMode` value, default implementation-defined (only when `agent.runtime=codex`)
- `codex.turn_sandbox_policy`: Codex `SandboxPolicy` value, default implementation-defined (only when `agent.runtime=codex`)
- `codex.turn_timeout_ms`: integer, default `3600000` (only when `agent.runtime=codex`)
- `codex.read_timeout_ms`: integer, default `5000` (only when `agent.runtime=codex`)
- `codex.stall_timeout_ms`: integer, default `300000` (only when `agent.runtime=codex`)
- `claude.command`: shell command string, default `claude --print --output-format stream-json --input-format stream-json --verbose` (only when `agent.runtime=claude`)
...
agent runtime. The specific protocol for the targeted agent runtime is the source of truth for
protocol schemas, message payloads, transport framing, and method names. The active runtime is
selected by `agent.runtime` (see §5.3.5); §10.1–§10.7 describe the Codex app-server runtime;
§10.8 describes the Claude stream-json runtime.

Protocol source of truth:

- Implementations MUST send messages that are valid for the targeted agent runtime's protocol.
- Implementations MUST consult the targeted agent runtime's documentation or generated schema
  instead of treating this specification as a protocol schema.
- If this specification appears to conflict with the targeted agent runtime's protocol, the runtime
...
### 10.8 Claude Stream-JSON Runtime

This subsection describes the `claude` coding-agent runtime, selected when `agent.runtime == "claude"`.

#### 10.8.1 Launch

- Command: resolved from `claude.command` (default:
  `claude --print --output-format stream-json --input-format stream-json --verbose`).
- Invocation: `bash -lc <claude.command>` in the workspace directory.
- If `claude.model` is set, append `--model <value>` to the effective command.
- If `claude.permission_mode` is set, append `--permission-mode <value>` to the effective command.
- RECOMMENDED max line size: 10 MB (for safe buffering).

#### 10.8.2 Prompt Input

- After process start, the runtime writes exactly one newline-delimited JSON message to stdin:

  ```json
  {"type":"user","message":{"role":"user","content":[{"type":"text","text":"<rendered prompt>"}]}}
  ```

- The message MUST be followed by a newline character and stdin MUST be closed (or the EOF signal
  sent) so the `claude` CLI begins processing.

#### 10.8.3 Event Mapping

Stdout is read as newline-delimited JSON. Each line is one JSON object. The runtime maps Claude
stream-json event types to Symphony's internal event surface (see §10.4) as follows:

| Claude event `type` / `subtype`   | Symphony event                  | Notes                                        |
|-----------------------------------|---------------------------------|----------------------------------------------|
| `system` / `init`                 | `EventThreadStarted`            | Extract `session_id` from the `session_id` field if present. |
| `assistant` / (any)               | `EventAssistantMessage`         | Payload carries assistant message content.   |
| `user` / (tool result)            | `EventToolResult`               | Carries tool execution result content.       |
| `result` / `success`              | turn completed                  | Extract token counts from `usage` field.     |
| `result` / `error_*`              | turn failed                     | Map subtype to a normalized error category.  |

- Lines that do not parse as valid JSON MUST be emitted as `malformed` events and MUST NOT abort
  the turn.
- Unknown `type` values MUST be emitted as `other_message` events and ignored for orchestration
  purposes.

#### 10.8.4 Token Accounting

- Token counts are extracted from the `usage` field of `result`-type events.
- Expected fields: `input_tokens`, `output_tokens`, `cache_read_input_tokens` (OPTIONAL),
  `cache_creation_input_tokens` (OPTIONAL).
- Accumulate totals in orchestrator state per §13.5.

#### 10.8.5 Session and Turn Identifiers

- `thread_id`: derived from the `session_id` field of the `system/init` event when available;
  otherwise generated by the implementation.
- `turn_id`: generated per turn by the implementation.
- `session_id`: composed as `<thread_id>-<turn_id>` per §4.2.

#### 10.8.6 Timeouts

- `claude.read_timeout_ms`: timeout for individual line reads from stdout during startup and turn
  streaming; enforced with the same semantics as `codex.read_timeout_ms` (see §10.6).
- `claude.turn_timeout_ms`: total wall-clock limit for one turn; enforced with the same semantics
  as `codex.turn_timeout_ms`.
...
This section defines the normative contract for all tracker adapters. All adapters MUST normalize
their output to the domain `Issue` model defined in §4.1.1, regardless of the backing store or
API. §11.1–§11.5 establish the interface contract and Linear-specific behaviour. §11.6–§11.8
define the additional tracker kinds introduced in §5.3.8–§5.3.10.
...
4. `create_comment(issue_id, body)`
   - Append a comment to the identified issue.
   - Behaviour is tracker-kind-specific; see §11.2 (Linear), §11.6 (Markdown), §11.7 (OpenSpec),
     §11.8 (JIRA).

5. `update_issue_state(issue_id, state)`
   - Transition the identified issue to the named state.
   - Behaviour is tracker-kind-specific; see §11.2 (Linear), §11.6 (Markdown), §11.7 (OpenSpec),
     §11.8 (JIRA).
...
### 11.6 Markdown Tracker (`tracker.kind == "markdown"`)

The Markdown tracker treats every `.md` file under `tracker.markdown.root` as one Issue. The
filesystem is the source of truth; no external API is required.

#### 11.6.1 Issue Source

- Recursively enumerate all `.md` files under the resolved `tracker.markdown.root` directory.
- Each file corresponds to one Issue.

#### 11.6.2 Field Derivation

Fields are derived as follows to satisfy the normalized Issue model (§4.1.1):

- `id`: SHA-256 hex digest of the absolute path of the file.
- `identifier`: filename stem (filename without the `.md` extension).
- `title`: the text of the first top-level Markdown heading (`# ...`) in the file body after
  front matter is stripped. If no H1 is present, fall back to `identifier`.
- `description`: the Markdown body with YAML front matter removed and leading/trailing whitespace
  trimmed.
- `state`: the value of the `state:` key in the YAML front matter. Default: `Todo` when the key
  is absent or empty.
- `url`: `file://` URI of the absolute path of the file.
- `labels`: normalized to lowercase from the `labels:` front-matter key (list of strings);
  defaults to an empty list.
- `priority`, `branch_name`, `blocked_by`, `created_at`, `updated_at`: sourced from the
  correspondingly named front-matter keys when present; otherwise null / empty.

#### 11.6.3 `create_comment` Semantics
...
The append MUST be atomic with respect to concurrent readers (use a write-then-rename strategy
or equivalent).

#### 11.6.4 `update_issue_state` Semantics

Rewrite the `state:` key in the YAML front matter of the file to the new state value. The rewrite
MUST be atomic: write to a temporary file in the same directory, then rename over the original.
Concurrent readers MUST observe either the old state or the new state, never a partially-written
file.

#### 11.6.5 State Mapping

- `active_states` and `terminal_states` from `tracker` config apply directly to the `state`
  front-matter value using case-insensitive comparison (per §4.2).

### 11.7 OpenSpec Tracker (`tracker.kind == "openspec"`)

The OpenSpec tracker maps `openspec-cli` change-proposal directories to Issues. Each
`openspec/changes/<slug>/` directory is one active Issue; directories under
`openspec/archive/<slug>/` are considered done.

#### 11.7.1 Issue Source

- Active issues: subdirectories of `<tracker.openspec.root>/changes/`.
- Archived/done issues: subdirectories of `<tracker.openspec.root>/archive/`.
- Each subdirectory name is a `slug` that serves as both `id` and `identifier`.

#### 11.7.2 Field Derivation

Fields are derived as follows to satisfy the normalized Issue model (§4.1.1):

- `id`: the slug (directory name).
- `identifier`: the slug.
- `title`: the text of the first top-level Markdown heading (`# ...`) in `proposal.md` within the
  slug directory. Falls back to the slug when no H1 is present.
- `description`: the body of `proposal.md` (front matter stripped) concatenated with the
  `## Tasks` section extracted from `tasks.md` (if `tasks.md` exists in the slug directory).
- `state`: `Todo` when the slug is under `changes/`; `Done` when the slug is under `archive/`.
- `url`: `file://` URI of the absolute path of `proposal.md` within the slug directory.
- `labels`: `["openspec"]` plus any additional labels from the `labels:` YAML front-matter key of
  `proposal.md`, normalized to lowercase.
- `priority`, `branch_name`, `blocked_by`, `created_at`, `updated_at`: sourced from the
  correspondingly named front-matter keys of `proposal.md` when present; otherwise null / empty.

#### 11.7.3 `create_comment` Semantics

Append the following block to the end of `proposal.md` within the slug directory:
...
#### 11.7.4 `update_issue_state` Semantics

- Transitioning to a terminal state (any state in `tracker.terminal_states`):
  `os.Rename(<root>/changes/<slug>)` → `<root>/archive/<slug>`. Creates the `archive/` directory
  if it does not exist.
- Transitioning to an active state (any state in `tracker.active_states`):
  `os.Rename(<root>/archive/<slug>)` → `<root>/changes/<slug>`. Creates the `changes/` directory
  if it does not exist.
- The rename MUST be atomic on the host filesystem.
- After renaming, if the target slug directory contains a `proposal.md` with a `state:` front-
  matter key, that key SHOULD also be updated to reflect the new state.

#### 11.7.5 State Mapping

- `Todo` maps to active (under `changes/`); `Done` maps to archived (under `archive/`).
...
Base64-encoded `email:api_token`.

#### 11.8.1 Issue Source

- Issues are fetched via `POST /rest/api/3/search` with JQL:
  `project = <project_key> AND status in (<active_states>)`.
- When `tracker.jira.jql_override` is set, that JQL string replaces the default query entirely.
- Pagination SHOULD be used; page size default: `50`.
- Network timeout: `30000 ms`.

#### 11.8.2 Field Derivation

Fields are derived as follows to satisfy the normalized Issue model (§4.1.1):

- `id`: JIRA internal issue `id` field (numeric string).
- `identifier`: JIRA issue `key` (for example `ABC-123`).
- `title`: `fields.summary`.
- `description`: `fields.description` converted from Atlassian Document Format (ADF) to plain
  text. Null when absent.
- `state`: `fields.status.name`.
- `url`: `fields.url` or constructed from `<endpoint>/browse/<key>`.
- `labels`: `fields.labels` normalized to lowercase strings.
- `priority`: `fields.priority.id` coerced to integer when present; otherwise null.
- `blocked_by`: derived from issue links where `type.inward == "is blocked by"`.
- `created_at`: `fields.created` parsed as ISO-8601 timestamp.
- `updated_at`: `fields.updated` parsed as ISO-8601 timestamp.

#### 11.8.3 `create_comment` Semantics

POST to `/rest/api/3/issue/{key}/comment` with an ADF body representing the comment text.
Implementations SHOULD convert plain-text `body` to a minimal ADF paragraph node. Surface any
`errors` returned by the API as a structured error.

#### 11.8.4 `update_issue_state` Semantics

1. GET `/rest/api/3/issue/{key}/transitions` to retrieve available transitions.
2. Find the transition whose `to.name` matches the target state (case-insensitive).
3. POST to `/rest/api/3/issue/{key}/transitions` with `{"transition":{"id":"<transition_id>"}}`.
4. If no matching transition is found, return an error without mutating the issue.

#### 11.8.5 Authentication

- Header: `Authorization: Basic base64(<email>:<api_token>)`.
- `$VAR` resolution applies to `tracker.jira.email` and `tracker.jira.api_token` per §6.1.
- If `api_token` resolves to an empty string after `$VAR` substitution, treat it as missing and
  fail preflight.
